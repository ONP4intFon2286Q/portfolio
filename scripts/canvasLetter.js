function drawLetter(letter, fontMulti, clicked) {
  ...
  canvasCon.style.maxWidth = "1440px";
  ...
  ctx.fillText(
    letter,
    canvas.width / 2,
    canvas.height / 2 + height / 2 - metrics.actualBoundingBoxDescent
  );
  ...
  for (var y = 0; y < imageData.height; y += partGridSpacing) {
    for (var x = 0; x < imageData.width; x += partGridSpacing) {
      ...
      var particle = {
        ...
        color: `rgba(${r + partColorR}, ${g + partColorG}, ${b + partColorB}, ${
          a + partColorA
        })`,
      };
  ...
  function partChangeCircles() {
    ...
    for (var i = 0; i < particles.length; i++) {
      ...
      ctx.arc(
        particle.x + partSize / 2,
        particle.y + partSize / 2,
        partSize / 1.5,
        0,
        2 * Math.PI
      );
